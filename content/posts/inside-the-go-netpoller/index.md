        ...
        -> (*internal/poll.pollDesc).wait()
            -> internal/poll.runtime_pollWait()
                -> runtime.netpollblock()
                    -> runtime.gopark()
```
5. Go runtime 在适当的时机（GC、调度 goroutine 以及 sysmon goroutine）轮询（`netpoll()`） IO 事件，发现该 socket 上发生读事件`EOLLIN`，设置该文件描述符的`rg`为`pdReady`，恢复（`netpollgoready()`）该 goroutine。
6. `work` goroutine 恢复执行，退出`runtime.netpollblock()`，退出`internal/poll.runtime_pollWait()`，返回底层系统调用`read(2)`的返回值，`conn.Read(b)`退出。
